...
@client.command()
async def level(ctx):
    levelbed = discord.Embed(title='Nível', description=f'{user.mention} se encontra atualmente no nível {level}', colour=discord.Color.red())
    ...
    if ctx.author.id == users[str(user.id)]['id']:
